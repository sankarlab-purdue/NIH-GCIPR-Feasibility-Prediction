...
MAXIMUM_NUMBER_UNMAPPED_PRODUCT_ATOMS = 5000
...
def get_tagged_atoms_from_mols(mols): # Match
...
def get_tagged_atoms_from_mol(mol): # Match
...
def atoms_are_different(atom1, atom2): # Different
...
def find_map_num(mol, mapnum): # New
...
def get_changed_atoms(reactants, products): # Different
...
def expand_atoms_to_use(mol, atoms_to_use, groups=[], symbol_replacements=[]): # Match
...
def convert_atom_to_wildcard(atom):
    ...
    #        # Fully generalize atom-mappped neighbors 
...
def get_fragments_for_changed_atoms(mols, changed_atom_tags, radius=0, 
    category='reactants', expansion=[], SUPER_GENERAL_TEMPLATES=False):
    ...
    for mol in mols:
        ...
        if category == 'reactants':
            groups = get_special_groups(mol, SUPER_GENERAL_TEMPLATES)
        else:
            groups = []
...
def extract_from_reaction(reaction):
    ...
    try:
        ...
        reactant_fragments, intra_only, dimer_only = get_fragments_for_changed_atoms(reactants, changed_atom_tags, 
            radius = 1, expansion = [], category = 'reactants', SUPER_GENERAL_TEMPLATES=False)
        ...
        product_fragments, _, _  = get_fragments_for_changed_atoms(products, changed_atom_tags, 
            ...
            category = 'products', SUPER_GENERAL_TEMPLATES=False)
...
# Slightly modified extract_from_reaction function !!

def extract_from_reaction_fwd(reaction):
    ...
    try:
        # Get fragments for reactants
        reactant_fragments, intra_only, dimer_only = get_fragments_for_changed_atoms(reactants, changed_atom_tags, 
            radius = 1, expansion = [], category = 'reactants', SUPER_GENERAL_TEMPLATES=True)
        # Get fragments for products 
        # (WITHOUT matching groups but WITH the addition of reactant fragments)
        product_fragments, _, _  = get_fragments_for_changed_atoms(products, changed_atom_tags, 
            radius = 1, expansion = expand_changed_atom_tags(changed_atom_tags, reactant_fragments),
            category = 'products', SUPER_GENERAL_TEMPLATES=True)
    except ValueError as e:
        if VERBOSE:
            print(e)
            #print(reaction['_id'])
