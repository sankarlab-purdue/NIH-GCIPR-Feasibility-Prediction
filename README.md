...
Retrosynthesis planning tools coupled with Feasibility Analyzer, as described in {name of publication}.
...
- **rdenzyme_file, retrobiocat_file:** Templates for RDEnzyme and RetroBioCat reactions
- **buyable_file:** List of commercially available compounds (due to GitHub size limit, please download the database here: )
- **custom_file:** User-defined compounds to treat as buyable (editable in data > change_endpoints)
- **excluded_file:** Commercially available compounds that user defined as "Not Buyable" (editable in data > change_endpoints)
...
- **build_directionality_dataset**: generates *data\12162025_directionality_dataset.pkl*. It is used for feasibility assessment.
- **build_rhea_cosubstrate_dataset**: generates *data\cofactor_by_id.csv*. It is used to generate the full reaction annotation, which is needed for feasibility assessment. 
- **extract_rdenzyme_rhea_templates**: generates *data\rhea_atom_mapped_for_jx_cache.pkl*. It contains the pre-extracted templates from RDEnzyme for cache building. 
...
- **DirDatasetBuilder**: Has the functions needed to generate the *data\12162025_directionality_dataset.pkl* in the *dataset_builder\build_directionality_dataset.ipynb* notebook.
- **FeasibilityAnalyzer**: Has the main functions needed for feasibility assessment of reactions. 
- **FeasibilityAnalyzer_multistep**: Has helper functions to perform feasibility assessment of multi-step pathways and their display
- **FeasibilityAnalyzer_singlestep**: Has helper functions to perform feasibility assessment of single-step reactions and their display
